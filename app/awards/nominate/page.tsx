...
function NominateContent() {
  ...
  useEffect(() => {
    ...
    const found = all.find(a => a.id === awardId) ?? null;
    setAward(found);

    // Load existing nominations
    const nomRaw = localStorage.getItem('fe_my_nominations');
    const noms: Nomination[] = nomRaw ? JSON.parse(nomRaw) : [];
    ...
    if (editId) {
      ...
      setAlreadyNominated(noms.some(n => n.awardId === awardId && !editId));
    }
  }, [awardId, editId]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const noms: Nomination[] = raw ? JSON.parse(raw) : [];

    if (isEditing && editId) {
      const updated = noms.map(n => n.id === editId
        ? { ...n, ...form, status: 'pending' as const, updatedAt: new Date().toISOString() }
        : n
      );
      ...
      localStorage.setItem('fe_my_nominations', JSON.stringify([...noms, newNom]));
    }
